...
- [ ] 新增 `configs/dataset.yaml`，记录数据根目录和输出目录。
- [ ] 新增 `configs/thresholds.yaml`，记录 TTC、漏检持续时间、控制延迟等阈值。
- [ ] 新增 `configs/llm.yaml`，记录是否启用 LLM、模型名、温度、JSON 输出约束。
- [ ] 新增 `data/README.md`，说明大数据不放 git，实际位置在 `/data5/lzx_data/Zhijia-Guardian`。
- [ ] `configs/llm.yaml` 第一版默认关闭 LLM：
...
- [ ] `conda activate yolo` 后能 `import torch, cv2, pandas, shapely, streamlit, langgraph`。
- [ ] `/data5/lzx_data/Zhijia-Guardian` 目录存在。
- [ ] 主仓目录结构清晰，不把大数据提交进 git。

## 2. P-1：6 个 canonical demo 样本

- [ ] 新增 `data/sample_scenarios/canonical_demo/`。
- [ ] 写 6 个最小 demo JSON。
- [ ] 每类 2 个：`perception_like_nuscenes`、`planning_like_nuplan`、`full_stack_like_carla`。
- [ ] 每个 demo 都使用 `source/meta/frames/events_observed/oracle`。
- [ ] 每个 demo 的 `oracle.visible_to_diagnosis=false`。

验收标准：

- [ ] 6 个样本能通过 schema 校验。
- [ ] 诊断入口只拿 observed view。
- [ ] 评估入口才读 oracle。

## 3. P0：Canonical Scenario Schema + ManualAdapter

- [ ] 实现 `src/schemas/scenario.py`。
- [ ] 实现 `src/schemas/metrics.py`。
- [ ] 实现 `src/schemas/diagnosis.py`。
- [ ] 实现 `src/adapters/base_adapter.py`。
- [ ] 实现 `src/adapters/manual_adapter.py`。
- [ ] schema 使用 Pydantic，所有字段带类型、默认值和校验。
- [ ] 支持读取单个 `.json` 场景和 `.jsonl` 批量场景。
- [ ] 场景 schema 必须是 `ScenarioRecord`：`scenario_id/source/meta/frames/events_observed/oracle`。
- [ ] 实现 `ScenarioRecord.observed_view()`，只返回诊断可见字段。
- [ ] 实现 `load_oracle()`，只允许 `experiments/run_eval.py` 调用。
- [ ] 增加 `tests/test_no_label_leakage.py`，确保诊断流程无法读取 `oracle`。
...
- [ ] `perception.detection_source`
- [ ] `perception.detections`
- [ ] `planning.available`
- [ ] `planning.trajectory_source`
- [ ] `planning.trajectory`
- [ ] `control.available`
- [ ] `map.available`
...
- [ ] `actors_gt_source` 只能是 `simulation`、`dataset_annotation`、`offline_reconstruction`、`unavailable`。
- [ ] `planning.trajectory_source` 只能是 `expert_future`、`offline_planner`、`perturbed_planner`、`model_prediction`、`unavailable`。
- [ ] `perception.detection_source` 只能是 `model_output`、`synthetic_from_annotation`、`dataset_prediction`、`unavailable`。
- [ ] `scenario_id`、文件名、目录名不得包含 `fault_type` 或 `root_module`；允许 `manual_v0_1_000001.json`，禁止 `perception_miss_001.json`。
...
- [ ] 能读取 1 个最小 demo 场景。
- [ ] 缺字段时报清楚错误。
- [ ] 能输出标准化内部 dict / Pydantic model。
- [ ] Rule-only、Single-LLM、Multi-Agent + Tools 的入口只接收 observed view。

## 4. P0.5：真实数据 Adapter Contract + Stub Adapters

- [ ] 新增 `docs/adapter_contract.md`。
- [ ] 新增 `docs/schema_mapping_nuscenes.md`。
- [ ] 新增 `docs/schema_mapping_nuplan.md`。
- [ ] 新增 `docs/schema_mapping_carla.md`。
- [ ] 实现 `src/adapters/nuscenes_stub_adapter.py`。
- [ ] 实现 `src/adapters/nuplan_stub_adapter.py`。
- [ ] 实现 `src/adapters/carla_stub_adapter.py`。
...
- [ ] `ManualAdapter`、`NuScenesStubAdapter`、`NuPlanStubAdapter`、`CarlaStubAdapter` 都能输出同一个 `ScenarioRecord`。
- [ ] 不同数据源缺失字段时，诊断流程能自动跳过对应 Agent。
- [ ] 所有工具只依赖 Canonical Scenario，不依赖原始数据格式。

## 5. P0.6：真实数据兼容的手工/脚本合成样本

- [ ] 写 `src/adapters/manual_adapter.py` 或 `scripts/generate_manual_scenarios.py`。
- [ ] 生成 60-100 个 Canonical Scenario JSON，先覆盖 5 类故障和 normal。
- [ ] 手工样本必须通过 Canonical Scenario Schema 生成，不允许临时字段。
- [ ] 所有生成样本记录 `generation_seed`、`noise_profile`、`scenario_family`、`difficulty`。
- [ ] 生成脚本支持 `--seed`、`--count`、`--output`。
- [ ] 手工样本保留 `source.raw_tokens` 字段，为后续真实数据 token 映射预留位置。
- [ ] `scenario_id` 和文件名统一使用无标签格式，如 `manual_v0_1_000001`。
...
- [ ] `perception_like_nuscenes`：有 ego、actors_gt、actors_gt_source、perception.detections；planning/control 为 `available=false`。
- [ ] `planning_like_nuplan`：有 ego、actors_gt、actors_gt_source、map、planning.trajectory_source、planning.trajectory；perception/control 可为 `available=false`。
- [ ] `full_stack_like_carla`：有 ego、actors_gt、perception、planning、control、events_observed。
- [ ] 每类样本都必须模拟真实数据可能出现的字段缺失。
...
- [ ] 时间噪声：fault_start_time、观测异常时间、风险出现时间加入 ±0.2s 偏移。
- [ ] 感知噪声：confidence 随机波动。
- [ ] 目标噪声：目标位置、速度轻微偏移。
- [ ] 控制噪声：brake/throttle/steer 延迟和抖动。
- [ ] 复合故障：感知轻微异常 + 规划响应不足。
- [ ] 边界样本：TTC 接近阈值但不一定故障。
...
- [ ] 所有样本能通过 schema 校验。
- [ ] 每个样本的 `oracle` 有真值 `fault_type`、`root_module`、`fault_start_time`。
- [ ] 传给诊断函数/Agent 的对象中不含 `oracle`。
- [ ] 三个 demo 有清晰时间线和可视化轨迹。

## 6. P0.7：真实数据最小 Adapter Smoke Test

目标：已经下载 nuScenes mini 和 nuPlan mini，因此在写大量 tools/agents 前，先验证 canonical schema 能否接真实数据。

- [ ] 写 `src/adapters/nuscenes_adapter.py` 的最小只读版本。
- [ ] 写 `src/adapters/nuplan_adapter.py` 的最小只读版本。
- [ ] nuScenes mini 抽 1 个 sample 转 `ScenarioRecord`。
- [ ] nuPlan mini 抽 1 个 scene/scenario 转 `ScenarioRecord`。
- [ ] 输出到 `data/sample_scenarios/real_smoke_test/`。
- [ ] 不跑诊断，不计算 Fault Macro-F1，只做 schema validate、observed view 检查、字段覆盖率统计。

nuScenes smoke test 要求：

- [ ] `actors_gt_source=dataset_annotation`。
- [ ] 当前 metadata-only 状态下 `perception.available=false`。
- [ ] `planning.available=false`。
- [ ] `control.available=false`。
- [ ] 不声称已经跑图像/点云 detector。

nuPlan smoke test 要求：

- [ ] `actors_gt_source=dataset_annotation`。
- [ ] `control.available=false`。
- [ ] `planning.trajectory_source=expert_future` 或 `unavailable`。
- [ ] `scenario_tag` 只能进入 `events_observed.context_tags` 或抽样条件，不能进入 `oracle`。
- [ ] 若后续生成危险规划轨迹，必须标注 `planning.trajectory_source=perturbed_planner`。

验收标准：

- [ ] 两个真实样本都能通过 schema 校验。
- [ ] 两个真实样本的 observed view 不包含 `oracle`。
- [ ] 输出字段覆盖率报告，说明哪些 Agent 会自动跳过。
...
- [ ] `src/tools/ttc.py`
- [ ] `src/tools/collision.py`
- [ ] `src/tools/perception_eval.py`
- [ ] `src/tools/planning_eval.py`
- [ ] `src/tools/control_eval.py`

具体指标：

- [ ] TTC 曲线、min TTC、TTC violation 起止时间。
- [ ] ego 与目标最小距离。
- [ ] 规划轨迹与障碍物 bbox/轨迹碰撞检测。
- [ ] 感知漏检检测：GT 存在但 perception 缺失。
- [ ] 感知误检检测：perception 存在但 GT 不存在。
- [ ] 类别混淆检测：GT 类别与 detection 类别不一致。
- [ ] 置信度突降检测：关键目标 confidence 下降。
- [ ] 控制延迟检测：风险/规划要求制动但 brake 延迟。
- [ ] 舒适性辅助指标：acceleration、jerk、yaw rate。
- [ ] 每个指标发现都生成结构化 evidence。
...
- [ ] 对每个场景输出 `metrics.json`。
- [ ] 指标工具不依赖 LLM。
- [ ] 每个指标都有单元测试或至少 demo 验证样例。
- [ ] 每条 evidence 有唯一 `evidence_id`。

## 8. P1：Rule-only baseline

- [ ] 实现 `experiments/baselines/rule_only.py`。
- [ ] 用固定规则直接输出故障类型、根因模块、故障开始时间。
- [ ] 实现 `experiments/metrics.py`。
- [ ] 实现 `experiments/run_eval.py`。
- [ ] `experiments/run_eval.py` 是唯一读取 `oracle` 的评估入口。
- [ ] 支持 `--method`、`--dataset`、`--run-id`、`--seed`、`--config`。
- [ ] 每次实验输出到 `/data5/lzx_data/Zhijia-Guardian/outputs/runs/{run_id}/`。
- [ ] 每次实验写入 `run_meta.json`。
...
- [ ] 能对 60-100 个手工样本跑出 CSV。
- [ ] 能输出混淆矩阵。
- [ ] Rule-only 作为后续多 Agent 的下限。
- [ ] 重复相同 seed 和配置能得到相同结果。
