...
本 Todo 以 `/home/lzx/Zhijia-Guardian/docx/design.md` 为基础。2026-07-05 的
`docs/research_reframing_sut_real2sim_agents.md` 负责 SUT/数据边界，`docs/agent_redefinition_v2.md` 负责
Agent 定义且优先级最高；旧文档与二者冲突时以新文档为准。
...
10. 现有显式 Pydantic DAG 冻结为 `deterministic_workflow_v1` baseline；Agent v2 因具备调查循环、异步反事实实验和人工批准，使用独立 LangGraph `InvestigationGraph`。
...
16. Agent 必须具备 goal、private state、hypothesis set、action space、tool allowlist 和 stop condition；固定 parser/calculator/checker/scorer 全部称为工具或 workflow node，不再包装成 Agent。
...
- [x] 已确认当前 typed DAG 是固定 workflow，不满足 Agent v2 定义；保留为 deterministic baseline。
- [x] 感知/规划/控制检查器降级为 Domain Investigator 可调用的 evidence tools，不再作为 Agent 本体。
- [ ] Agent v2 必须围绕 hypothesis board 迭代，主动选择取证、证伪、反事实、优化或拒答动作。
- [ ] 增加 Critic 和 Counterfactual gate；未经证伪与反事实验证的 Top-1 只能叫候选原因。
- [ ] 增加 Optimization + Validation 闭环；未经回归验证的内容只能叫优化建议，不能叫已验证优化。
...
## 9. P2：Legacy Deterministic Workflow v1

说明：当前 `diagnosis_graph.py` 是固定顺序、纯规则的模块化 workflow，不再称为最终 Multi-Agent 方法。
它继续承担 deterministic baseline、无 LLM fallback 和 tools/schema 集成回归。

框架 commit `838ba17` 已将顺序函数升级为显式 Pydantic DAG。该图满足 fan-out/fan-in、条件跳过、
完整 trace 和 oracle 隔离，但没有 hypothesis/action/tool feedback loop，因此按
`docs/agent_redefinition_v2.md` 归档为 `deterministic_workflow_v1`。
...
- [x] legacy Report Composer 使用确定性模板；LLM 报告仅保留为可选实验，不作为第一版验收项。
- [x] 无证据时必须输出 `uncertain` 或 `skipped`，不能硬猜。
- [x] 第一版默认 LLM 关闭，Multi-Agent + Tools 必须在纯规则模式下可运行。
- [x] legacy 模块节点严禁读取 `oracle`。
- [x] 图初始化从 `observed_view()` 重建 ScenarioRecord，传给 workflow node 的对象中 `oracle=None`、`source.generation={}`。
...
- [x] 旧固定 workflow 不引入 LangGraph，继续使用 Pydantic DAG。
- [x] Agent v2 的 LangGraph checkpoint/interrupt 工作转移到 P2.1，不要求与 legacy `diagnosis_v1` 完全同构。
...
- [x] legacy workflow 每个节点结果可以在 Streamlit 中展示。
- [x] 报告中每个结论能反查到 metrics evidence。
- [x] 每个 claim 都有 `claim_id` 和 `evidence_ids`。
- [x] legacy Planning Node 相邻车道误报回归已覆盖：control-delay/normal 不再被轨迹圆形包络误判为 planning risk。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Deterministic workflow v1（历史名 Multi-Agent + Tools） | 1.0000 | 1.0000 | 1.0000 | 1.0000 | 0.0000 | 1.0000 | 0.0000 |
...
说明：这里的满分属于旧固定 workflow 在可控 synthetic benchmark 上的机制验证，不能作为 Agent v2
效果，也不代表自然事故泛化。
Single-LLM 分类准确，但幻觉率仍高于 0.10 目标，产品默认保持 LLM 关闭。

## 9.1 P2.1：Agent v2 InvestigationGraph

目标：实现真正能够提出假设、主动取证、证伪、请求反事实实验、生成优化候选并回归验证的多智能体闭环。

### A. Agent 与状态契约

- [ ] 新增 `src/zhijia_guardian/schemas/investigation.py`。
- [ ] 定义 `AgentAction`：proposal/tool/delegate/evidence/counterfactual/challenge/intervention/validation/abstain/finalize。
- [ ] 定义 `Hypothesis`：mechanism、predicted observations、required evidence、falsifiers、confidence、status。
- [ ] 定义 `InvestigationTask`、`CounterfactualRun`、`OptimizationCandidate`、`ValidationResult`。
- [ ] 定义 `InvestigationState`：goal、coverage、hypotheses、causal graph、tasks、budgets、trace、stop reason。
- [ ] 所有 confidence 更新必须引用新 evidence/counterfactual ID。
- [ ] Agent 输入对象物理移除 `fault_oracle`；LLM action schema 禁止 oracle 字段。

### B. 工具与旧节点迁移

- [ ] `metric_agent` 重命名/包装为 deterministic metric service。
- [ ] `scene_agent` 迁移为 coverage/triage tool。
- [ ] perception/planning/control fixed scorer 迁移为 Domain Investigator 的 evidence tools。
- [ ] visual review 迁移为可选视觉工具；只有 Investigator 显式调用才运行。
- [ ] CARLA/nuPlan replay 注册为异步 experiment tools，返回 run_id，不把大结果塞进 prompt。
- [ ] report/failure-sample 迁移为 renderer 和 regression service。

### C. Agent 角色

- [ ] Case Manager Agent：调查目标、hypothesis board、委派、预算和阶段切换。
- [ ] Domain Investigator Agent：按需加载 perception/planning/control profile，循环选择工具并寻找反证。
- [ ] Causal Reasoner Agent：生成带 evidence_id 和时间的候选因果图。
- [ ] Critic Agent：主动攻击 Top-1，检查替代解释、泄漏、循环论证和不可测试假设。
- [ ] Counterfactual Experiment Agent：选择最小干预，提交 paired replay，比较首次分歧和结果。
- [ ] Optimization Agent：提出 versioned config/code proposal、预期收益、副作用和验证计划。
- [ ] Validation Agent：执行故障 A/B 与 healthy regression，输出 accepted/rejected/inconclusive。
- [ ] Report Composer Service：只能序列化已存在状态，不能在最后一步创造新根因或优化结论；不计入 Agent。

### D. 编排与安全边界

- [ ] 在 `yolo` 环境增加版本固定的 LangGraph 可选依赖，不重复安装 Torch。
- [ ] 新增独立 `InvestigationGraph`，legacy `DiagnosisGraph` 保持不变。
- [ ] 实现 investigation -> causal -> critic -> investigation/counterfactual 循环。
- [ ] 实现 optimization -> validation -> optimization/finalize 循环。
- [ ] 默认预算：最多 3 轮调查、2 次 counterfactual、3 个 optimization candidate；耗尽后 `ABSTAIN`。
- [ ] CARLA 运行、自动 patch proposal 应用和高成本 VLM 调用前支持 interrupt/approval。
- [ ] 使用 checkpoint + thread_id 保存状态，工具副作用必须幂等。
- [ ] DeepSeek 作为首版 policy model，temperature=0、结构化 action、完整 prompt/model/tool/config hash。

### E. 第一批 smoke

- [ ] 选择 1 条 perception -> planning -> control 复合故障，只给 observed SUT 输出，不给根因标签。
- [ ] Case Manager 至少提出 2 个可证伪 hypothesis，而不是直接输出一个分类。
- [ ] Investigator 至少自主选择 2 个不同工具，Critic 至少否决或削弱 1 个替代原因。
- [ ] Counterfactual Agent 请求关闭上游注入的 paired replay，并正确读取 first divergence。
- [ ] Optimization Agent 生成 1 个最小干预，Validation 在 fault case + healthy regression 上执行。
- [ ] 输出 `investigation.json`、`causal_graph.json`、`optimization.json`、`validation.json` 和最终报告。

验收标准：

- [ ] action trace 能证明下一步动作随 observation 改变，不是预写死相同节点序列。
- [ ] 未通过 Critic 的 hypothesis 不得 FINALIZE；未通过 Validation 的优化不得标记 verified。
- [ ] 工具失败、预算耗尽和数据不足均能 checkpoint 并安全输出 `uncertain/not_diagnosable`。
- [ ] 同一 action trace 可 replay；核心工具结果可复现，LLM 成本和延迟可统计。
...
目标：证明 Agent 的假设调查、证伪、反事实和优化闭环是否产生价值，不再用“把固定函数拆成多个节点”作为
Agent 实验。

对比方法：

- [ ] `rule_only`：简单规则下限。
- [ ] `deterministic_workflow_v1`：当前固定模块 workflow。
- [ ] `single_llm`：一次性读取摘要直接回答。
- [ ] `agent_v2_no_critic`：Critic 消融。
- [ ] `agent_v2_no_counterfactual`：反事实消融。
- [ ] `agent_v2_diagnosis_only`：Optimization/Validation 消融。
- [ ] `agent_v2_full`：完整调查与优化方法。

数据子集：

- [ ] seen single fault：验证基础工具调用。
- [ ] unseen compound fault：验证 hypothesis Recall@K 与跨模块传播。
- [ ] missing/contradictory evidence：验证取证选择和 abstention。
- [ ] real-calibrated CARLA domain shift：验证真实 profile 下的稳定性。
- [ ] optimization regression：每个 fault case 配套 healthy 和其他故障回归集。

诊断指标：

- [ ] Hypothesis Recall@1/3、Verified Root Accuracy、Fault Time MAE。
- [ ] Falsification Precision：Critic 否决的 hypothesis 中真正错误的比例。
- [ ] Evidence Coverage/Correctness、Hallucination Rate、Abstention Accuracy。
- [ ] Counterfactual Success：有效最小干预实验占比。

优化指标：

- [ ] Repair Success Rate、风险/TTC 改善、碰撞或违规下降。
- [ ] Healthy Regression Rate、其他故障回归失败率。
- [ ] 修改规模、回滚成功率和未验证建议比例。

Agent 效率指标：

- [ ] 平均 investigation rounds、tool calls、counterfactual runs、token、延迟和 API 成本。
- [ ] 无关工具调用率、重复调用率、checkpoint resume 成功率。

判定规则：

- [ ] 只有 `agent_v2_full` 相对 no-critic/no-counterfactual 在 unseen/compound 上提升，才声称协作调查有效。
- [ ] 只有优化通过 fault A/B 且 healthy regression 不退化，才声称系统完成“诊断与优化”。
- [ ] 不把 LangGraph、Agent 数量、trace 长度或模型生成文本量当作性能提升证据。
...
- [x] `deterministic_workflow_v1`（历史名 Multi-Agent + Tools）baseline。
- [ ] Agent v2 调查、反事实与优化主方法。
- [x] 实验结果 CSV。
- [x] 混淆矩阵和对比表。
...
- [x] Rule-only、Single-LLM、deterministic workflow v1 三组历史结果能通过统一 comparison 包对比。
...
- [ ] 同一异常案例能够产生至少两个可证伪 hypothesis，并根据工具 observation 改变后续路径。
- [ ] Critic、Counterfactual、Optimization、Validation 四个 gate 均有结构化输出和失败状态。
- [ ] 至少一个 CARLA 故障完成“诊断 -> 最小干预 -> paired replay -> healthy regression”闭环。
- [ ] 完整 Agent v2 与 no-critic、no-counterfactual、diagnosis-only 消融完成统一比较。
- [ ] 最终报告严格区分 candidate、verified root、suggested optimization 和 verified optimization。
