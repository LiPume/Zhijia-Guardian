...
本节是工程口径，优先级高于后文中偏申报书化、偏理想化的设计描述。2026-07-05 新增的 Agent v2
重新定义优先级最高，完整契约见 `docs/agent_redefinition_v2.md`；后文旧的模块 Agent 设计只作为
`deterministic_workflow_v1` 历史基线。

### 0.0 Agent v2 强制修订

旧定义把 Scenario Parser、Metric Calculator、Perception Checker、Planning Checker、Control Checker 都称为
Agent。它们执行预定函数，实际上是工具和模块诊断节点，不构成本项目最终的多智能体创新点。

新版规定：Agent 必须同时拥有 goal、private state、hypothesis set、action space、tool allowlist 和 stop
condition，并能根据新 observation 改变下一步动作。核心协作过程从“并行模块检查”改成：

```text
异常事件 + 明确 SUT 日志
  -> Case Manager 建立调查目标与 hypothesis board
  -> 按需委派 Domain Investigator 主动取证
  -> Causal Reasoner 构建候选传播图
  -> Critic 寻找反证和替代解释
  -> Counterfactual Agent 请求 CARLA/nuPlan paired replay
  -> Optimization Agent 生成最小可回滚干预
  -> Validation Agent 做 A/B 和 healthy regression
  -> Report Composer Service 输出已验证根因、未决项和优化结果
```

感知、规划、控制不再天然各对应一个固定 Agent，而是 Investigator 的专家 profile 和可调用工具。只有与当前
hypothesis 相关的专家才执行。旧 Pydantic DAG 保留为固定流程 baseline 和无 LLM fallback；Agent v2 因为
存在循环、异步实验、checkpoint 和人工批准，使用独立 LangGraph `InvestigationGraph`。
...
```text
异常场景日志 + SUT provenance
  -> 多 Agent 假设调查与主动取证
  -> 因果证伪和反事实重放
  -> 优化候选与回归验证
  -> 根因报告、优化报告、失败样本包、Streamlit 展示
```
...
2. 能定位故障从什么时候开始。
3. 能输出有指标证据支撑的根因报告。
4. 能提出可执行优化候选，并用 paired replay 证明风险下降且 healthy regression 不退化。
...
| 多智能体诊断层 | LangGraph 编排多个 Agent | 需要重新定义 | 旧 Pydantic DAG 降级为 baseline；Agent v2 使用 hypothesis/tool/counterfactual 循环，不采用自由聊天 |
| 根因与优化层 | 主因、副因、证据链、优化建议 | 可行但必须验证 | Critic 证伪、paired replay 验因，Optimization + Validation 形成闭环 |
...
2026-07-05 修订：当前显式 Pydantic DAG 继续覆盖 typed state、fan-out/fan-in、条件跳过、oracle 隔离
和完整 trace，但它被重新归类为 `deterministic_workflow_v1`。Agent v2 已经包含调查循环、异步
counterfactual、optimization validation 和人工批准，因此新增独立 LangGraph `InvestigationGraph`，
不再要求它与 legacy `diagnosis_v1` 完全同构。极端天气第一版已在 CARLA 实跑重雨、浓雾和夜间风暴三组共 12 条；
...
> 基于 CARLA 构造带 fault oracle 的 SUT 闭环异常场景，Agent v2 围绕 hypothesis 主动取证、证伪并请求
> paired replay，随后生成最小优化候选并执行 healthy regression。旧 Pydantic DAG 作为确定性 baseline，
> LangGraph `InvestigationGraph` 负责动态调查、反事实和优化验证状态。
...
多智能体诊断层采用 Agent v2 定义，不采用自由聊天，也不再把固定 checker 称为 Agent。每个 Agent 必须
维护可证伪 hypothesis，依据 observation 选择结构化 action 和工具，并拥有预算与停止条件。

核心 Agent 包括：

1. Case Manager Agent：维护调查目标、hypothesis board、预算和委派。
2. Domain Investigator Agents：按需加载感知/规划/控制/定位等专家 profile，主动取证和找反证。
3. Causal Reasoner Agent：构建带时间和证据的候选因果图。
4. Critic Agent：攻击 Top-1、检查泄漏、循环论证和替代解释。
5. Counterfactual Experiment Agent：请求 paired replay 和最小干预实验。
6. Optimization Agent：生成最小、可回滚、可验证的优化候选。
7. Validation Agent：执行 A/B 和 healthy regression，接受或拒绝优化。
8. Report Composer Service：只序列化已验证结论、未决问题和优化结果；因无独立 hypothesis/action loop，不计入 Agent。

Scenario parser、metric calculator、perception/planning/control fixed scorer、可视化和 failure sample builder
全部归为工具或服务。Domain Investigator 可以调用它们，但不能把一次固定函数调用伪装成智能体协作。
...
Agent v2 主框架：LangGraph `InvestigationGraph`
固定流程 baseline：Pydantic `DiagnosisGraph`
可选 LLM SDK：OpenAI-compatible client

分工如下：

| 技术                  | 用途                                   |
| ------------------- | ------------------------------------ |
| Pydantic typed DAG  | `deterministic_workflow_v1` baseline、无 LLM fallback |
| LangGraph | Agent v2 调查循环、subgraph、checkpoint、interrupt/resume 和异步实验 |
| OpenAI-compatible client | Agent policy、Single-LLM baseline；工具计算仍由 Python 完成 |
...
### 5.2 为什么旧 workflow 不需要、Agent v2 需要 LangGraph

旧版是状态化、多步骤、可审计的固定诊断 workflow，Pydantic DAG 已足够表达其预定流程：
...
Agent v2 仍禁止自由聊天，但它会根据 observation 选择 hypothesis、工具和实验，并在 Critic、
Counterfactual、Optimization、Validation 之间循环。此时 checkpoint、interrupt/resume、异步任务回填和
状态 fork 已成为实际需求，因此采用独立 LangGraph。框架本身仍不是效果变量，效果必须来自证伪、反事实
或优化验证消融。
...
> 2026-07-05 修订：以下 10.1-10.8 是旧版固定流程组件说明，不再作为 Agent v2 的定义。其中 Parser、
> Metric Calculator、模块 scorer、Report renderer 和 Data Generation 均迁移为工具/服务。新版 Agent
> 契约、角色、动作与状态机见 `docs/agent_redefinition_v2.md`。
...
用显式 typed state graph 串联诊断流程；LangGraph backend 仅在持久化/中断恢复需求出现后增加。
...
本项目应做成“自动驾驶异常调查与优化工作台”：针对明确版本 SUT，Agent v2 提出可证伪假设、主动调用
Python 工具取证、由 Critic 攻击候选、用 CARLA paired replay 验证因果，再提出最小干预并执行 healthy
regression。旧 typed graph 作为 baseline；LangGraph 负责循环和持久化，但不是效果提升来源。

项目不要一开始追求完整车企级平台，而应先证明四件事：

1. 系统能诊断出故障属于哪一类；
2. 系统能定位故障从什么时候开始；
3. 系统能给出有证据链的解释报告。
4. 系统能提出一个优化候选，并经反事实和回归实验判断它是否有效。
