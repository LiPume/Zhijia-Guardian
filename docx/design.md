...
2026-07-01 补充决策：当前显式 Pydantic DAG 已覆盖 typed state、fan-out/fan-in、条件跳过、
oracle 隔离和完整 trace，不把 LangGraph 作为 MVP 强依赖。只有加入工程师审核中断/恢复、持久化
checkpoint、跨进程重试或 time-travel 后，才增加可选 LangGraph backend，并要求两种 backend
输出相同的 `diagnosis_v1`。极端天气第一版已在 CARLA 实跑重雨、浓雾和夜间风暴三组共 12 条；
...
> 基于 CARLA 构造带有真值标签的异常驾驶场景，使用显式状态图编排多个诊断 Agent，结合 Python 指标工具计算 TTC、碰撞风险、轨迹偏离、感知漏检等指标，最终输出结构化根因诊断报告，并在测试集上评估故障分类 F1、根因 Top-1 准确率和故障时间定位误差。MVP 使用 Pydantic DAG，LangGraph 仅作为需要持久化和人工中断恢复时的可选 backend。
...
多智能体诊断层采用显式 typed state graph 编排，不采用自由聊天式 Agent。每个 Agent 都是一个明确的诊断节点，必须基于指标工具输出结构化结果。当前 backend 是 Pydantic DAG；LangGraph 只在需要 checkpoint、interrupt/resume 或跨进程执行时启用。
...
主框架：Pydantic typed DAG
可选编排 backend：LangGraph
...
| 技术                  | 用途                                   |
| ------------------- | ------------------------------------ |
| Pydantic typed DAG  | MVP 多 Agent 状态图、节点依赖、条件跳过和结构化校验        |
| LangGraph（可选）    | 需要 checkpoint、interrupt/resume、time-travel 时使用 |
| OpenAI-compatible client（可选） | Single-LLM baseline 和可选报告润色 |
...
### 5.2 为什么当前不强制采用 LangGraph

本项目不是普通问答系统，而是一个状态化、多步骤、可审计的诊断流程。每个节点都要保留输入、指标、证据和结论。当前 Pydantic DAG 已能显式定义流程图，例如：
...
如果直接用自由式 Agent，流程可能变成一个大模型自由判断，容易出现证据链不稳定、步骤不可控、报告不可复现的问题。LangGraph 的实际价值在持久化、人工中断恢复和长任务容错；在这些需求落地前增加它不会改变当前诊断结果，因此暂不作为实验变量。
...
本项目第一阶段应做成一个“自动驾驶异常场景诊断工作台”：基于 CARLA 构造有真值标签的异常场景，用显式 typed graph 编排多智能体诊断流程，用 Python 工具计算 TTC、碰撞、漏检、误检、轨迹偏离等客观指标，最终输出可解释根因报告，并通过故障分类 F1、根因 Top-1 准确率和故障时间定位误差证明系统有效。LangGraph 是产品进入持久化人工审核流程后的可选工程增强，不是当前效果提升来源。
