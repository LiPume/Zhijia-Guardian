...
> 2026-07-05 方法命名修订：本报告中的历史名称 `Multi-Agent + Tools` 指固定 Pydantic
> `deterministic_workflow_v1`，用于证明模块证据与时序排序机制，不代表新定义的 Agent v2。Agent v2
> 必须包含 hypothesis、主动取证、Critic、Counterfactual、Optimization 和 Validation，尚未实现，不能
> 使用本报告数值宣称 Agent v2 效果。定义见 `docs/agent_redefinition_v2.md`。
...
1. 在带已知根因的 manual/CARLA 受控故障中，deterministic workflow v1 能利用模块异常的时间先后，
...
| nuScenes six-camera YOLO v0.2 | 2 个 scene、12 个相机片段、486 帧 | 真实六相机 + YOLOv8n | 分相机/距离召回、无标签诊断 | 3D 融合、Fault/Root Accuracy |
...
六相机扩展使用同一投影和 Canonical contract，将每个“scene×camera”作为独立诊断记录，避免同一
物体跨视角重复计为 false positive。运行 `scripts/run_nuscenes_multicamera_benchmark.py --clean`
可生成 3×2 同步视频、逐相机指标和 0-20m/20-40m/40m+ 距离分桶召回。它仍是六路独立 2D 推理，
不是 LiDAR/相机 3D 融合。
...
| 指标 | CAM_FRONT v0.1 | Six-camera v0.2 |
| --- | ---: | ---: |
| 真实 scene / 相机片段 / 帧数 | 5 / 5 / 202 | 2 / 12 / 486 |
| Annotation Recall | 0.4706 | 0.4845 |
| 关键目标 Recall | 0.5391 | 0.5525 |
| Detection Precision | 0.7248 | 0.7174 |
| 匹配目标类别准确率 | 0.9290 | 0.9420 |
| Fault/root oracle | 无 | 无 |
...
六相机 v0.2 的 annotation recall 随距离由 0-20m 的 0.7261 降至 20-40m 的 0.4572 和 40m+ 的
0.2782。12 个相机片段均输出 `perception_miss` 假设，Planning/Control Agent 各跳过 12 次。这说明
系统能暴露 detector 的距离退化并按缺失字段降级，但不能证明 12 个根因判断正确。
...
3. **真实标签缺失：** nuScenes 没有被测 planner/control 和 root oracle，只能评价 perception 兼容性；六相机 v0.2 也不是 3D 融合模型。
