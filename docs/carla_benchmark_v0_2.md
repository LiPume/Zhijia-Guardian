# CARLA Fault Benchmark v0.2-riskfix

> 2026-07-01 修订：历史 `fault_benchmark_v0_2` 在 control-delay 注入时使用最低 TTC 时刻，而当前
> 指标使用首次 TTC 阈值穿越，导致一条 oracle 与实际制动响应矛盾。本文结果已从相同 base logs
> 重新生成到 `fault_benchmark_v0_2_riskfix`；历史 commit `7b3cafd` 的结果不再进入主表。
...
Rule-only 和无时序消融的 10 个全量错误均发生在 composite variant：5 个 perception miss 和 5 个
confidence drop 被下游高权重 `control_delay` 覆盖。完整 Multi-Agent 按模块分诊断，并在上游证据
早于下游至少 0.25 秒时提升上游候选，因此恢复正确根因。两个 boundary-normal variant 均未误报。

最终 run 目录：

```text
/data5/lzx_data/Zhijia-Guardian/outputs/runs/final_carla_fault_riskfix_{rule,no_temporal,full}_seed42/
/data5/lzx_data/Zhijia-Guardian/outputs/runs/final_carla_fault_riskfix_test_{rule,no_temporal,full}_seed42/
```
